...
Setting $k = 2$ we get that such probability is a bit greater than $\frac{1}{2}$ and so the solution is feasible and with cost less than $6*log(n)*LP$ with probability a bit less than $\frac{1}{2}$ ($\frac{1}{2} - \frac{1}{2*n}$).
...
\[
    ...
    max\text{ }y                & & max\text{ }v \\
    4*x_1 - 2*x_2 \geq y    & & -4*u_1 + u_2 + v \leq v \\
    -x_1 + 2*x_2 \geq y     & & 2*u_1 - 2*u_2 + v \leq v \\
    x_1 + x_2 = 1               & & u_1 + u_2 = 1 \\
